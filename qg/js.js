window.onerror=function(msg,url,line){$.post("https://tracking.jake4maths.com/generatorerror.php",{msg:encodeURIComponent(msg),url:encodeURIComponent(url),line:encodeURIComponent(line)});console.log('there was an error');return !1}
...
$('#selectors').load("allquestions.html?cachebuster="+new Date().getTime(),function(){if(window.location.hash.length){questions=window.location.hash.substring(1).split(',');window.location.hash='';checknewquest()}
...
function processquestion(w,e){if(e==1){$.get("https://tracking.jake4maths.com/newgeneratorget.php",{question:w});var _href=$("#linkprint").attr("href")+w+',';$('#linkprint').attr("href",_href);var _href=$("#linkscreen").attr("href")+w+',';$('#linkscreen').attr("href",_href);var _href=$("#linkcreator").attr("href")+w+',';$('#linkcreator').attr("href",_href);window.location.hash+=w+','}
var array=window[w]();if(typeof array==='object'){var question=array.question;var answer=array.answer;var description=array.description;var lastdescription=$('.questiondescription').last().text();if(lastdescription!=$('<div>'+description+'</div>').text()){$('#questions').append('<li class=questiondescription>'+description+'</li>')}
hideworking=''
...
function algebraanswer(answer){return '<div contenteditable="true" class=checkalgebra answer="'+CaesarCipher(answer,1)+'"></div><br><div class="algebradisplay"></div>'}
function checkalgebra(){$('.checkalgebra').bind('blur keyup paste copy cut mouseup',function(){var math=$(this).parent().children('.algebradisplay').first();math.css('opacity','0');math.html('`'+$(this).text()+'`');math.css('opacity','1')})}
function numberanswer(answer){return '<div contenteditable="true" class=checknumber answer="'+encode64(parseFloat(answer))+'"></div>'}
function dragdropanswer(answer){answer=answer.split('|');a=[];answer.forEach(function(entry){a.push(encode64(entry))});a=a.join('|');return '<div class=drop id=drop'+randint(0,10000000000)+' ondrop="drop(event)" ondragover="allowDrop(event)" answer="'+(a)+'"></div>'}
function dragdropdrops(entry){return '<div class=draghome id=drop'+randint(0,10000000000)+' ondrop="drop(event)" ondragover="allowDrop(event)"><div class=drag id=drag'+randint(0,10000000000)+' draggable="true" ondragstart="drag(event)">'+entry+'</div></div><br>'}
function strReplaceAll(string,Find,Replace){try{return string.replace(new RegExp(Find,"gi"),Replace)}catch(ex){return string}}
$(document).on('click','#checkall',function(){$(".check").each(function(){$(this).trigger("click")})});$(document).on('click','.check',function(){var done='yes';var answers=0;var algebracorrect='no';var attempts=$(this).parent().children('.attemptholder').first().children('.attempt').first().text();$(this).parent().children('.questiontext').first().find('.checktext, .checknumber, .checkmulti ,.checkalgebra, .drop, .select').each(function(){answers=answers+1;if($(this).text().replace(/\s+/g,'').length==0){thiscurrentlyis=""}else if($(this).hasClass('checknumber')){var thiscurrentlyis=encode64(parseFloat(parseFloat($(this).text()).toPrecision(3)))}else if($(this).hasClass('checkmulti')){var thiscurrentlyis=encode64($(this).text().replace(/\s+/g,''))}else if($(this).hasClass('checkalgebra')){var thiscurrentlyis=$(this).text();thiscurrentlyis=algebra.parse(thiscurrentlyis);thisshouldbe=algebra.parse(CaesarCipher($(this).attr("answer"),-1));algebraanswer=thiscurrentlyis.subtract(thisshouldbe).toString();if(algebraanswer==0){algebracorrect='yes'}}else if($(this).hasClass('select')){var thiscurrentlyis=encode64($(this).val())}else{var thiscurrentlyis=encode64($(this).text())}
var thisshouldbe=$(this).attr("answer").split("|");if(thisshouldbe.indexOf(thiscurrentlyis)>-1||algebracorrect=='yes'){if($(this).hasClass('select')){$(this).replaceWith('<div class="checkeda">'+$(this).val()+'</div>')}
$(this).removeClass('checktext');$(this).removeClass('checknumber');$(this).removeClass('checkmulti');$(this).removeClass('checkalgebra');$(this).removeClass('checkedwrong');$(this).addClass('checkeda');$(this).find('[draggable]').removeAttr('draggable');$(this).removeAttr('contenteditable').blur()}else{if(attempts==3){if($(this).hasClass('select')){$(this).replaceWith('<div class="checkedwrong">'+$(this).val()+'</div>')}}
$(this).addClass('checkedwrong');done='no'}});if(done=='yes'||attempts==3){$(this).parent().children('.attemptholder').first().text('Correct on Attempt '+attempts+'/3');$(this).removeClass('check');$(this).addClass('checked');$(this).removeAttr('style');$(this).parent().children('.questiontext').first().children('.buttons').first().remove();$(this).parent().children('.questiontext').first().children('.checktext, .checknumber, .checkmulti, .checkalgebra').each(function(){if($(this).text().length==0){$(this).text('not answered')}
$(this).removeAttr('contenteditable').blur();if($(this).hasClass('checknumber')){$(this).after("<br><div style='display:inline-block;font-size:80%;'>Should have been "+decode64($(this).attr("answer"))+'</div>')}})
...
$(this).parent().remove();$('#showhideanswers').remove();$('#answerholder').remove()});$(document).on('keypress','.checktext, .checknumber, .checkmulti, .checkalgebra',function(e){return e.which!=13});function rand(min,max,step){var num=new BigNumber.random();num=(max-min)*num+min;num=Math.floor(num/step);num=num*step;num=parseFloat(num).toPrecision(Math.max(Math.floor(Math.log(1/step)),1));return new BigNumber(num)}
...
function preventmove(){$('.checktext,.checknumber,.checkmulti,.working,.checkalgebra').bind('blur keyup paste copy cut mouseup',function(){isChange=!0})}
...
function addchar(item){item.parent().siblings('.checktext, .checknumber, .checkmulti, .checkalgebra').first().append(item.html().substring(1));setEndOfContenteditable(item.parent().siblings('.checktext, .checknumber, .checkmulti, .checkalgebra').first().get(0));isChange=!0}
function clearbox(item){item.parent().siblings('.checktext, .checknumber, .checkmulti, .checkalgebra').first().text(" ");setEndOfContenteditable(item.parent().siblings('.checktext, .checknumber, .checkmulti, .checkalgebra').first().get(0))}
function setEndOfContenteditable(contentEditableElement){var range,selection;if(document.createRange){range=document.createRange();range.selectNodeContents(contentEditableElement);range.collapse(!1);selection=window.getSelection();selection.removeAllRanges();selection.addRange(range)}
else if(document.selection){range=document.body.createTextRange();range.moveToElementText(contentEditableElement);range.collapse(!1);range.select()}}
function CaesarCipher(str,num){str=str.toLowerCase();var result='';var charcode=0;for(i=0;i<str.length;i++){charcode=(str[i].charCodeAt())+num;result+=String.fromCharCode(charcode)}
return result}
...
return array}
function patterntable(width,height,home,filled){table="<table class='clickers'>";i=1;r=0;while(r<height){table+='<tr>';c=0;while(c<width){table+='<td class="'+i;console.log(home);if($.inArray(i,filled)>-1){table+=' clicked'}
if($.inArray(i,home)>-1){table+=' home'}
table+='">';console.log(i);c++;i++}
r++}
table+='</table>';return table}
function transgrid(id){var canvas=$("#"+id)[0];var ctx=canvas.getContext('2d');ctx.fillStyle='#fff'
ctx.fillRect(0,0,500,300);ctx.strokeStyle="#888";ctx.lineWidth=2;drawgrid(ctx,10,490,10,290);$("#"+id).click(function(e){var ctx=$(this)[0].getContext('2d');var rect=$(this)[0].getBoundingClientRect();var x=e.clientX-rect.left,y=e.clientY-rect.top
x=Math.floor((x-10)/20)*20+20;y=Math.floor((y-10)/20)*20+20;if(y<20||x<20||x>480||y>280){return}
var c=ctx.getImageData(x,y,1,1).data;if(c[0]<180){ctx.fillStyle='#ffffff'}else{ctx.fillStyle='#00A400'}
ctx.fillRect(x-9,y-9,18,18)})}
function colorsquare(x,y,id){var ctx=$('#'+id)[0].getContext('2d');ctx.fillStyle='#0066A4'
ctx.fillRect(x-9,y-9,18,18)}
function colordot(x,y,id){var ctx=$('#'+id)[0].getContext('2d');ctx.beginPath();ctx.arc(x,y,5,0,Math.PI*2,!0);ctx.fillStyle='#F00'
ctx.fill();ctx.closePath()}
function colorline(x,y,xx,yy,id){var ctx=$('#'+id)[0].getContext('2d');ctx.strokeStyle="#000";ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(xx,yy);ctx.stroke();ctx.strokeStyle="#888";ctx.lineWidth=2}
